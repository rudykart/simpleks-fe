<template>
   <div class="modal fade" id="createPersediaanBahanBaku" data-bs-backdrop="static" data-bs-keyboard="false"
      tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
      <!-- <loader v-if="loading" /> -->
      <div class="modal-dialog">
         <div class="modal-content">
            <div class="modal-header">
               <h1 class="modal-title fs-5" id="staticBackdropLabel">Tambah Persediaan Bahan Baku</h1>
               <button type="button" @click="createOpenCloseFun()" class="btn-close" data-bs-dismiss="modal"
                  aria-label="Close"></button>
            </div>
            <form @submit.prevent="_createDataPersediaanBahanBaku">
                  ...
                  <div v-for="(stokBahanBaku, index) in data.stokBahanBakus" :key="stokBahanBaku">
                     <div class="row">
                        <div class="col">
                           <div class="my-2">
                              <!-- <select v-model="stokBahanBaku.bahan_baku_id"
                                 ...
                                 <option v-for="(bahanBaku, index) in dataBahanBakus" v-bind:value="bahanBaku.id">
                                    {{ bahanBaku.nama_bb }}
                                 </option>
                              </select> -->
                              <select v-model="stokBahanBaku.bahan_baku_id"
                        ...
                        <div class="col">
                           ...
                           <div v-if="validation['stokBahanBakus.' + index + '.stok_bb']" style="margin-top: -7px;"
...
export default {
   ...
   methods: {
      ...mapActions('persediaanBahanBaku', ['createDataPersediaanBahanBaku']),
      _resetForm() {
         this.data = {
            keterangan_pbb: '',
            stokBahanBakus: [
      ...
      createOpenCloseFun() {
         $('#createPersediaanBahanBaku').modal('hide');
         this._resetForm()
      },
      ...
      //    }
      // },
      async _createDataPersediaanBahanBaku() {
         // console.log(this.data)
         this.loading = true
         try {
            await this.createDataPersediaanBahanBaku(this.data)
      ...
      add() {
         this.data.stokBahanBakus.push({ bahan_baku_id: null, stok_bb: null })
      },
