<template>
   <div class="modal fade" id="createKue" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
      aria-labelledby="staticBackdropLabel" aria-hidden="true">
      <!-- <loader v-if="loading" /> -->
      <div class="modal-dialog">
         <div class="modal-content">
            <div class="modal-header">
               <h1 class="modal-title fs-5" id="staticBackdropLabel">Tambah Kue</h1>
               <button type="button" @click="createOpenCloseFun()" class="btn-close" data-bs-dismiss="modal"
                  aria-label="Close"></button>
            </div>
            <form @submit.prevent="_createDataKue">
                  ...
                  <div class="row">
                     ...
                     <!-- <div class="col">
                        <div class="mb-2">
                           ...
                           <input v-model="data.keuntungan_kue" type="text"
                              :class="[validation.keuntungan_kue ? 'form-control is-invalid' : 'form-control']">
                           <span v-if="validation.keuntungan_kue" class="form-text text-danger">
                     ...
                     <div class="col">
                        ...
                        <div v-if="validation.keuntungan_kue" style="margin-top: -7px;" class="form-text text-danger">
                           {{ validation.keuntungan_kue[0] }}
                        </div>
                     </div>
                  </div>
                  ...
                  <div v-for="(komposisiKue, index) in data.komposisiKues" :key="komposisiKue">
                     <div class="row">
                        <div class="col">
                           <div class="my-2">
                              <select v-model="komposisiKue.bahan_baku_id"
                                 ...
                                 <!-- <option v-for="(bahanBaku, index) in bahanBakus.data" v-bind:value="bahanBaku.id">
                                    {{ bahanBaku.nama_bb }}
                                 </option> -->
                                 <option v-for="(bahanBaku, index) in bahanBakus" v-bind:value="bahanBaku.id">
                                    {{ bahanBaku.nama_bb }}
                                 </option>
                              </select>
                           </div>
                           <div v-if="validation['komposisiKues.' + index + '.bahan_baku_id']" style="margin-top: -7px;"
                        ...
                        <div class="col">
                           ...
                           <div v-if="validation['komposisiKues.' + index + '.jumlah_bb']" style="margin-top: -7px;"
                              class="form-text text-danger">
...
export default {
   ...
   methods: {
      ...
      //    }
      // },
      async _createDataKue() {
         // console.log(this.data)
         try {
            this.loading = true
            await this.createDataKue(this.data)
      ...
      add() {
         this.data.komposisiKues.push({ bahan_baku_id: null, jumlah_bb: null })
      },
